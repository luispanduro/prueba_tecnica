...
export class RolesProxyController {
  ...
  constructor(
    ...
    this.roleServiceUrl = this.config.get<string>('ROLE_SERVICE_URL')!;
  }
  ...
  async forward(
    ...
    const subPath = req.path.replace(/^\/api\/roles/, '');
    const qs = req.url.includes('?') ? '?' + req.url.split('?').slice(1).join('?') : '';
    const targetUrl = `${this.roleServiceUrl}/roles${subPath}${qs}`;
